...
async fn apply_cors_headers(bucket: &str, method: &http::Method, headers: &HeaderMap) -> Option<HeaderMap> {
    ...
    // Convert HTTP method to string for matching
...
impl S3 for FS {
    ...
    async fn get_object(&self, req: S3Request<GetObjectInput>) -> S3Result<S3Response<GetObjectOutput>> {
        ...
        if let Some(cors_headers) = apply_cors_headers(&bucket, &req.method, &req.headers).await {
...
mod tests {
    ...
    fn test_cors_headers_validation() {
        ...
        let allowed_headers = vec!["*".to_string()];
        ...
        let allowed_headers = vec!["content-type".to_string(), "x-custom-header".to_string()];
        ...
        let req_headers = vec!["content-type".to_string(), "x-forbidden-header".to_string()];
        let allowed_headers = vec!["content-type".to_string()];
        let all_allowed = req_headers
            .iter()
            .all(|req_header| allowed_headers.iter().any(|allowed| allowed.to_lowercase() == *req_header));
    ...
    fn test_cors_response_headers_structure() {
        ...
        let methods = vec!["GET", "PUT", "POST"];
        let allowed_headers = vec!["Content-Type", "Authorization"];
        let expose_headers = vec!["ETag", "x-amz-version-id"];
        let max_age = 3600;
