...
struct DistDeleteBatchLockLease {
    ...
    commit_locks_by_object: std::collections::BTreeMap<String, rustfs_lock::LockId>,
...
impl DistDeleteBatchLockLease {
    fn empty(clients: Vec<Arc<dyn LockClient>>) -> Self {
        Self {
            ...
            commit_locks_by_object: std::collections::BTreeMap::new(),
            clients,
...
fn spawn_dist_delete_batch_lock_heartbeat(
    ...
    tokio::spawn(async move {
        let mut lock_ids_by_client = Vec::with_capacity(heartbeat_entries_by_client.len());
        let heartbeat_entries_by_client: Vec<Vec<_>> = heartbeat_entries_by_client
            .into_iter()
            .map(|entries| {
                let mut lock_ids = Vec::with_capacity(entries.len());
                let entries = entries
                    .into_iter()
                    .map(|(object, lock_id, expires_at)| {
                        lock_ids.push(lock_id);
                        (object, expires_at)
                    })
                    .collect();
                lock_ids_by_client.push(lock_ids);
                entries
            })
            .collect();
        ...
        let mut confirmed_until: Vec<Vec<_>> = heartbeat_entries_by_client
            .iter()
            .map(|entries| entries.iter().map(|(_, expires_at)| *expires_at).collect())
            .collect();
        ...
        loop {
            ...
            let refreshes = join_all(
                clients
                    .iter()
                    .zip(&lock_ids_by_client)
                    .map(|(client, lock_ids)| client.refresh_locks_batch(lock_ids)),
            );
            let results = select! {
                _ = cancel.cancelled() => return,
            ...
            for (client_index, entries) in heartbeat_entries_by_client.iter().enumerate() {
                let client_results = results.get(client_index).and_then(|result| result.as_ref().ok());
                for (entry_index, (object, _)) in entries.iter().enumerate() {
                    match client_results.and_then(|results| results.get(entry_index)) {
                        ...
                        None if now >= confirmed_until[client_index][entry_index] => {
                            *unavailable_by_object.entry(object.clone()).or_default() += 1;
                        }
                        None => {}
                    }
...
impl ObjectLockDiagGuard {
    ...
    fn commit_lock(&self) -> disk::error::Result<Option<rustfs_lock::LockId>> {
        match self.guard.lock_id() {
            Some(_) => self
                .guard
                .commit_lock_id()
                .cloned()
                .map(Some)
                .ok_or_else(|| DiskError::other("distributed namespace lock has no consistent backend commit id")),
            None => Ok(None),
        }
    }
        ...
        for (req_idx, req) in batch.requests.iter().enumerate() {
            ...
            match resolution_by_object[req_idx] {
                ObjectLockResolution::Succeeded => {
                    let Some(commit_lock) = lock_ids_by_object[req_idx].first().map(|(_, lock_id, _)| lock_id.clone()) else {
                        failed_map.insert(
                            (req.key.bucket.as_ref().to_string(), req.key.object.as_ref().to_string()),
                            "distributed lock quorum returned no commit lock".to_string(),
                        );
                        continue;
                    };
                    if lock_ids_by_object[req_idx]
                        .iter()
                        .any(|(_, lock_id, _)| lock_id != &commit_lock)
                    {
                        ...
                        failed_map.insert(
                            (req.key.bucket.as_ref().to_string(), req.key.object.as_ref().to_string()),
                            "distributed lock quorum returned inconsistent commit locks".to_string(),
                        );
                        continue;
                    }
                    ...
                    lease
                        ...
                        .insert(req.key.object.as_ref().to_string(), commit_lock);
...
mod tests {
    ...
    impl LockClient for BatchRefreshCountingClient {
        ...
        async fn refresh_locks_batch(&self, lock_ids: &[rustfs_lock::LockId]) -> rustfs_lock::Result<Vec<bool>> {
            self.batches.fetch_add(1, Ordering::Relaxed);
            self.refreshed.fetch_add(lock_ids.len(), Ordering::Relaxed);
            Ok(vec![true; lock_ids.len()])
        }
    ...
    async fn test_rename_data_join_error_rolls_back_destination_object() {
        ...
        let result =
            SetDisks::rename_data(&disks, (RUSTFS_META_TMP_BUCKET, tmp_object), file_infos.into(), (bucket, object), 2, None)
                .await;
    ...
    #[tokio::test(start_paused = true)]
    async fn test_dist_delete_batch_lock_heartbeat_refreshes_thousand_keys() {
        ...
        assert_eq!(batches.load(Ordering::Relaxed), 1, "one locker must receive one heartbeat batch");
