...
impl DistributedLockGuard {
    ...
    /// Refresh errors are tolerated only until the last confirmed lease expires;
    /// loss never releases while the guarded operation is running.
        ...
        loop {
            ...
            for (index, result) in results.iter().enumerate() {
                match result {
                    ...
                    // An expired last confirmation fences a potentially stale writer.
    ...
    /// Return the exact backend id admitted by every quorum member.
    pub fn commit_lock_id(&self) -> Option<&LockId> {
        let (first, _) = self.entries.first()?;
        self.entries.iter().all(|(lock_id, _)| lock_id == first).then_some(first)
    }
...
impl DistributedLock {
    ...
    pub(crate) async fn acquire_guard(&self, request: &LockRequest) -> Result<Option<DistributedLockGuard>> {
        ...
        // Start TTL at acquisition start to avoid overestimating the oldest lease.
...
mod tests {
    ...
    async fn heartbeat_keeps_distributed_lock_refreshed() {
        ...
        assert_eq!(
            guard.commit_lock_id(),
            guard.entries.first().map(|(lock_id, _)| lock_id),
            "storage fencing must expose the exact id admitted by every backend"
        );
        assert_ne!(
            guard.commit_lock_id(),
            Some(&request.lock_id),
            "the acquisition attempt must use a fresh id instead of reusing the caller's template id"
        );
        assert_ne!(
            guard.lock_id(),
            &request.lock_id,
            "the public aggregate id must remain distinct from the backend commit id"
        );
